...
impl PathFilter {
    ...
    fn load_ignore_file<P: AsRef<Path>>(&self, path: P) -> GuardianResult<Vec<FilterPattern>> {
        let content = fs::read_to_string(&path)
            .map_err(|e| GuardianError::config(format!("Failed to read ignore file '{}': {}", 
...
mod tests {
    ...
    fn test_include_override() {
        ...
        assert!(!filter.should_analyze(Path::new("target/debug/lib.rs")).unwrap());
        assert!(filter.should_analyze(Path::new("target/special/lib.rs")).unwrap());
    }
    ...
    fn test_pattern_order() {
        ...
        assert!(filter.should_analyze(Path::new("tests/important.rs")).unwrap());
    }
