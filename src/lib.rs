...
pub use domain::violations::{
    Violation, Severity, ValidationReport, ValidationSummary, 
    GuardianError, GuardianResult
};

pub use config::{
    GuardianConfig, PatternRule, RuleType, PatternCategory
};

pub use analyzer::{
    Analyzer, AnalysisOptions, PatternStats
};

pub use report::{
    ReportFormatter, OutputFormat, ReportOptions
};

pub use cache::{
    FileCache, CacheStatistics
};
...
impl GuardianValidator {
    ...
    async fn analyze_with_cache(&mut self, paths: &[PathBuf], options: &AnalysisOptions) -> GuardianResult<ValidationReport> {
        ...
        for path in paths {
            if path.is_file() {
                ...
                let discovered_files: std::collections::HashSet<PathBuf> = temp_report.violations.iter()
        ...
        if let Some(cache) = &mut self.cache {
            ...
            if !files_to_analyze.is_empty() {
                ...
                for file_path in &files_to_analyze {
                    let file_violations: Vec<_> = all_violations.iter()
                        .filter(|v| v.file_path == *file_path)
                        .collect();
                    
                    if let Err(e) = cache.update_entry(file_path, file_violations.len(), &config_fingerprint) {
...
pub async fn validate_files<P: AsRef<Path>>(
    files: Vec<P>
) -> GuardianResult<ValidationReport> {
...
pub mod agent {
    ...
    pub async fn pre_commit_check<P: AsRef<Path>>(
        modified_files: Vec<P>
    ) -> GuardianResult<()> {
        let mut validator = GuardianValidator::new()?;
