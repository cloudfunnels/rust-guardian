...
impl Visit<'_> for ArchitecturalViolationVisitor {
    fn visit_item_use(&mut self, use_item: &syn::ItemUse) {
        ...
        if use_tree_str.contains("substrate") && !self.is_allowed_substrate_access() {
            ...
            .with_position(1, 1)
            .with_suggestion(
                "Consider accessing substrate through designated services or repositories",
            );
...
impl ArchitecturalViolationVisitor {
    fn is_allowed_substrate_access(&self) -> bool {
        ...
        path_str.contains("tests/") || path_str.contains("examples/")
    }
}
...
"#;
        ...
        assert!(violations
            .iter()
            .any(|v| v.rule_id.contains("unimplemented")));
...
"#;
    ...
    fn test_cdd_compliance_check() {
        ...
        let violations = analyzer
            .analyze(Path::new("src/main.rs"), content_without_header)
            .unwrap();
...
"#;
        let violations = analyzer
            .analyze(Path::new("src/main.rs"), content_with_header)
            .unwrap();
        assert!(!violations.iter().any(|v| v.rule_id == "cdd_header_missing"));
    }
    ...
    fn test_invalid_rust_syntax() {
        ...
        let violations = analyzer
            .analyze(Path::new("invalid.rs"), invalid_content)
            .unwrap();
