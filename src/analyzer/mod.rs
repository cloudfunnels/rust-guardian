...
use crate::domain::violations::{ValidationReport, Violation, GuardianError, GuardianResult};
use crate::patterns::{PatternEngine, PathFilter};
...
impl Analyzer {
    ...
    pub fn new(config: GuardianConfig) -> GuardianResult<Self> {
        ...
        for (category_name, category) in &config.patterns {
            ...
            for rule in &category.rules {
                ...
                pattern_engine.add_rule(rule, effective_severity)
    ...
    pub fn analyze_file<P: AsRef<Path>>(&self, file_path: P) -> GuardianResult<Vec<Violation>> {
        ...
        let content = fs::read_to_string(file_path)
            .map_err(|e| GuardianError::analysis(
        ...
        let matches = self.pattern_engine.analyze_file(file_path, &content)
            .map_err(|e| GuardianError::analysis(
                file_path.display().to_string(),
        ...
        if self.rust_analyzer.handles_file(file_path) {
            let rust_violations = self.rust_analyzer.analyze(file_path, &content)
                .map_err(|e| GuardianError::analysis(
                    file_path.display().to_string(),
    ...
    fn analyze_files_sequential(&self, files: &[PathBuf], options: &AnalysisOptions) -> GuardianResult<Vec<Violation>> {
        let mut all_violations = Vec::new();
    ...
    fn analyze_files_parallel(&self, files: &[PathBuf], options: &AnalysisOptions) -> GuardianResult<Vec<Violation>> {
...
mod tests {
    ...
    fn test_directory_analysis() -> GuardianResult<()> {
        ...
        let todo_violations: Vec<_> = report.violations.iter()
