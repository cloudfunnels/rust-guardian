...
impl FileCache {
    ...
    pub fn save(&mut self) -> GuardianResult<()> {
        ...
        if let Some(parent) = self.cache_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| GuardianError::cache(format!("Failed to create cache directory: {}", e)))?;
        }
    ...
    pub fn needs_analysis<P: AsRef<Path>>(&mut self, file_path: P, config_fingerprint: &str) -> GuardianResult<bool> {
        ...
        let metadata = fs::metadata(file_path)
            .map_err(|e| GuardianError::cache(format!("Failed to get file metadata for {}: {}", file_path.display(), e)))?;
    ...
    pub fn statistics(&self) -> CacheStatistics {
        CacheStatistics {
            ...
            hit_rate: if self.data.metadata.hits + self.data.metadata.misses > 0 {
                (self.data.metadata.hits as f64) / ((self.data.metadata.hits + self.data.metadata.misses) as f64)
            } else {
                0.0
            },
    ...
    fn calculate_file_hash<P: AsRef<Path>>(&self, file_path: P) -> GuardianResult<String> {
        ...
        loop {
            let bytes_read = file.read(&mut buffer)
                .map_err(|e| GuardianError::cache(format!("Failed to read file for hashing: {}", e)))?;
    ...
    fn migrate_if_needed(&mut self) -> GuardianResult<()> {
        ...
        if self.data.version < CURRENT_VERSION {
            tracing::info!("Migrating cache from version {} to {}", self.data.version, CURRENT_VERSION);
